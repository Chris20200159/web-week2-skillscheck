...
function bigOrSmall(arr){
	for (let answers = 0; answers >= 100; i++){
		console.log('big')
	} else {
		console.log('small')
	}
...
function arrayReverser(arr){
	for(let [reversed[i]] = array.length -1; i >= 0; i--){
		arr[i]
	}
	return arr;
};
...
const filtered = myNumbers.filter(number => number.valueOf >= 100)
...
const forTheLoveOfGeorge = function(name) {
	return notGeorge.map(x => {
		return {
			...x,
			nameType: "George"
		};
...
let people = (arr) => {
	let totallyAwesome = arr.reduce((accumulator, currentValue) => {
		let total = accumulator += currentValue.awesomeLevel
		return total
	}; 0)
